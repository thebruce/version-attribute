...
module.exports = class versionAttribute {
   ...
   *   version paths, one per member, in order.
   */
  getPreviousClosestVersion(pathCrumbs, tree) {
    let ascendPathCrumbs;
    let tempPathCrumbs;
    // Try to ascend - will give us the first real path above our target.
    ascendPathCrumbs = this.ascendPath(pathCrumbs, tree);
    ...
    while (tempPathCrumbs.length !== 0) {
      ...
      tempPathCrumbs = this.descendPath(ascendPathCrumbs, tree);
      // Test for [].
   ...
   */
  getVersionHasTarget(pathCrumbs, tree, target) {
    ...
    if (tempTarget) {
      ...
      ascendPathCrumbs = this.getPreviousClosestVersion(pathCrumbs, tree);
      if (!(_.isEqual(ascendPathCrumbs, []))) {
        // If we have a path we have what we need to move forward.
        answerArray = this.getVersionHasTarget(ascendPathCrumbs, tree, target);
      }
    }
